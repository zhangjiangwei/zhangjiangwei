...
var appNameList = new Array('circus','flopmachine','line','guess','kadang','littleapple','rotarytable','circus2','smallmahjong','niuniu','crazyrace','test_poker','test_texas','tanzhuji','sanguo','sangong','slot','fish','shaizile','texas','21dian','biyibi','zcsm','circusAndroidPad');
...
Client.loginStatus = function() {
	//1.7及以后版本，用h5判断是否登录
	if(Client.getJsVersion() >= 4 && userStatus != ""){
...
Client.logout = function(){
	...
	// if(Client.getJsVersion() >= 4){
        $.ajax({
            type:"POST",
            url:"http://gmall.wanlitong.com/?act=ucenter_logout&format=json",
            dataType:"jsonp",
            data:{},
            cache:false
        });
...
Client.getDeviceInfo = function(){
	if(typeof(PAG_JS.getDeviceInfo) != "undefined") {
		...
		return {
			imei:"860312025372168",
			imsi:"460018871305454",
			sdk:"16",
			screen:"720*1080",
...
Client.alipay = function(pay_amount,orderId){
	...
	if(typeof(PAG_JS.alipay) != 'undefined'){
		...
		window.location.href = '?act=alipaywap&out_trade_no='+orderId+'&total_fee='+amount;
	}
}
...
Client.openUrlInApp = function (url, userInfo, webpageTag) {
    if(Cookie.getCookie('newucenter') == 'newucenter'){
    ...
    if(indexOfHttp >= 0) {
        ...
        thisurl = "http://"+host[1]+"/"+thisurl;
    }
    
   var hostArray = ["gameapp.24caipiao.com",
   			window.location.host,
            "gameapp.wanlitong.com"];
...
function buryPoint(parterId){
    var _paq = {site:'gameapp',domain:window.location.host};
    _paq.userId = parterId;
    _paq.userType = 1;
    var $script = $('<script />');
    if(location.protocol.toLowerCase() == "http:"){
        $script.attr('src', '//h1.jkimg.net/webstat/pa_beacon_cdn.js').attr('type','text/javascript')
            .appendTo($('body'));
    }else{
        $script.attr('src', '//webstat.wanlitong.com/js/pa_beacon_https.js').attr('type','text/javascript')
            .appendTo($('body'));
    }
}
